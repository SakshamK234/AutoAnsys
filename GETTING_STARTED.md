...
The default `.env` values work with Docker Compose out of the box — no changes needed for local dev.
...
| `JWT_SECRET` | Secret for signing JWTs | (must set) |
| `CLUSTER_HOST` | HPC login node hostname | (for cluster mode) |
| `CLUSTER_USER` | SSH username for cluster | (for cluster mode) |
| `CLUSTER_KEY_PATH` | Path to SSH private key | (for cluster mode) |
