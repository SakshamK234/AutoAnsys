...
#SBATCH --output=/scratch/<user>/autoansys/jobs/<full_car_job>/slurm-%j.out
#SBATCH --error=/scratch/<user>/autoansys/jobs/<full_car_job>/slurm-%j.err
...
echo "Workspace:     /scratch/<user>/autoansys/jobs/<full_car_job>"

cd /scratch/<user>/autoansys/jobs/<full_car_job>
module reset
...
HOSTFILE="/scratch/<user>/autoansys/jobs/<full_car_job>/hostfile.txt"
...
# Solver stage: -gu -driver null (headless software rendering). With plain -g
# the graphics subsystem is removed and `display save-picture` does not exist —
# contour export always fails (verified on ARC, probes 6388030/6388066).
FLUENT_MODE_FLAG="-gu -driver null"
...
echo "Result files:"
ls -lh /scratch/<user>/autoansys/jobs/<full_car_job>/
echo "JOB_COMPLETE"
